    ...
    <svg className="spiral-svg" width="32" height="100%" viewBox="0 0 32 400" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: '32px', height: '100%', display: 'block' }}>
      <defs>
        ...
        <filter id="spiralShadow" x="-4" y="-4" width="40" height="408" filterUnits="userSpaceOnUse">
          <feDropShadow dx="0" dy="2" stdDeviation="2" floodColor="#a78bfa55" />
        </filter>
      </defs>
      ...
      {[...Array(18)].map((_, i) => (
        <ellipse key={i+100} cx="16" cy={18 + i * 21} rx="4" ry="3.5" fill="#b4b4b4" filter="url(#spiralShadow)" />
      ))}
    </svg>
  );
}
...
const navTitleStyle = {
  ...
  color: '#7c3aed',
  letterSpacing: '2px',
  textShadow: '0 2px 12px #a78bfa33',
  userSelect: 'none',
  lineHeight: 1,
  cursor: 'pointer',
    ...
    <ul style={{ width: '90%', maxWidth: 420, margin: '0 auto', padding: 0, listStyle: 'none', textAlign: 'center' }}>
      {items.map((pt, i) => (
        <li key={pt.title} style={{
          ...
          <div style={{ fontWeight: 800, fontSize: '1.15em', marginBottom: '0.3em', color: '#7c3aed' }}>{pt.title}</div>
          <div style={{ fontWeight: 500, color: '#4b5563', fontSize: '1em', maxWidth: 420 }}>{pt.desc}</div>
        </li>
      ))}
    </ul>
  );
...
function ExploreNow({ onBackToMain }) {
  ...
  if (page === 1) {
    leftContent = (
      <div style={{ width: '100%', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
        <div style={{ fontSize: '3.2rem', fontWeight: 900, color: '#7c3aed', letterSpacing: '2px', marginBottom: '0.5em', fontFamily: 'Impact, Charcoal, Arial Black, sans-serif' }}>Converza</div>
        <div style={{ fontSize: '1.5rem', color: '#a78bfa', fontWeight: 600 }}>Connect. Speak. Grow.</div>
      </div>
    );
    rightContent = null;
  } else if (page === 2) {
    leftContent = (
      <div style={{ width: '100%', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
        <div style={{ fontSize: '2rem', fontWeight: 800, color: '#7c3aed', marginBottom: '1em', fontFamily: 'Impact, Charcoal, Arial Black, sans-serif' }}>Why Converza?</div>
    ...
    leftContent = (
      <div style={{ width: '100%', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
        <div style={{ fontSize: '2rem', fontWeight: 800, color: '#7c3aed', marginBottom: '1em', fontFamily: 'Impact, Charcoal, Arial Black, sans-serif' }}>How to Use Converza?</div>
      ...
      <div style={navbarStyle}>
        ...
        <div ref={profileRef}>
          <button
            ...
            aria-label="Profile"
          >
            <svg height="24" width="24" viewBox="0 0 24 24" fill="none" stroke="#7c3aed" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {canFlipForward && (
            ...
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                <circle cx="12" cy="12" r="11" fill="#ede9fe" />
                <path d="M9 7l5 5-5 5" stroke="#7c3aed" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <div className="tooltip" style={{ opacity: 0, transition: 'opacity 0.2s', position: 'absolute', bottom: 32, right: 0, left: 'auto', background: '#7c3aed', color: '#fff', padding: '6px 14px', borderRadius: '8px', fontSize: '0.95rem', whiteSpace: 'nowrap', boxShadow: '0 2px 8px #a78bfa33', zIndex: 20, pointerEvents: 'none' }}>
                Click here to flip
              </div>
            </div>
          )}
